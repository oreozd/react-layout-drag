...
// class BannerView1 extends Component {
// 	constructor(props) {
// 		super(props)
// 		//页面渲染需要的数据
// 		this.state = {
...
const BannerView1 = (props) => {
	const { list } = useContext(AppContext);
	const {index, length, moveOn, moveDown, ondelete} = props;
	const data = list[index].BannerControl //banner配置项

	const tileBannerRender = (item) => {
		const colNum = props.modal === 'web' ? item.picMaxnum4Web : item.picMaxnum4App//web每行最多展示
		const len = item.bannerProperty.length || 1 //banner的总个数 //这个时候为啥是这样显示的
			...
			output.push(
				<span style={{width: `calc(100% / ${colNum})`, display: 'inline-block'}}>
					<Image 
						src={P1} 
						rounded  
						style={{height: props.modal === 'web' ? '200px' : '150px', position: 'relative', width: '100%'}} 
					/>
				</span>
			)
		}
